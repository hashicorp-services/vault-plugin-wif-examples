...
# Public OIDC issuer URL so Azure can fetch the discovery doc + JWKS to verify tokens.
...
resource "vault_secrets_sync_association" "demo" {
  name        = vault_secrets_sync_azure_destination.this.name
  type        = vault_secrets_sync_azure_destination.this.type
  mount       = vault_mount.kv.path
