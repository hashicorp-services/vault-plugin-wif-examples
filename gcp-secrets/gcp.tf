...
resource "google_iam_workload_identity_pool_provider" "vault_plugin_wif_provider" {
  ...
  attribute_condition = "assertion.sub == \"plugin-identity:root:secret:${vault_gcp_secret_backend.gcp.accessor}\""
}
...
resource "google_service_account_iam_member" "vault_plugin_wif_member" {
  ...
  member             = "principal://iam.googleapis.com/${google_iam_workload_identity_pool.vault_plugin_wif_pool.name}/subject/plugin-identity:root:secret:${vault_gcp_secret_backend.gcp.accessor}"
}
