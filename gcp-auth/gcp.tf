...
resource "google_iam_workload_identity_pool_provider" "vault_plugin_wif_provider" {
  ...
  attribute_condition = "assertion.sub == \"plugin-identity:${var.vault_namespace_id}:auth:${vault_gcp_auth_backend.plugin_wif.accessor}\""
}
...
resource "google_service_account_iam_member" "vault_plugin_wif_member" {
  ...
  member             = "principal://iam.googleapis.com/${google_iam_workload_identity_pool.vault_plugin_wif_pool.name}/subject/plugin-identity:${var.vault_namespace_id}:auth:${vault_gcp_auth_backend.plugin_wif.accessor}"
}


resource "google_project_iam_custom_role" "vault_plugin_wif_gcp_auth" {
